...
Replace the hardcoded PL011 base with a base read from the FDT stdout/serial node.
Keep the `uart_put` / `uart_str` API so nothing downstream changes.
...
## Immediate next step
Get an **RK3588 SBC** (or run QEMU) and boot SemOS on it — the FDT parser (M1) reads
its device tree, proving the runtime-discovery path on real-ish hardware before a Mac.
Then M2 (UART-from-FDT) → M4/M5 (memory) → M11A (virtio-net) on QEMU aarch64.
