...
| M4: Self-Repair | Agent detects its own failure (panic log), writes patch, compiles, installs — minimal human intervention | ✅ DONE 2026-08-21 (DEMO 88 QEMU approve+deny; head1: crash on zero-byte input → fault sentinel → diagnose → patch → verify → approved repair) |
