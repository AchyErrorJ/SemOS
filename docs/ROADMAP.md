...
| Phase 15 leapfrog — iPhone tether enumerated + data path 2026-06-10 | **Standalone EHCI driver + ipheth control plane confirmed ON HARDWARE + bulk data path landed.** The ~30-build xHCI PLS=Polling saga resolved by strategy flip: new `usb/ehci.rs` (per-controller async schedules — the shared-head QH was the W540 killer —, QH/qTD control+bulk, Rate-Matching-Hub hub-class enumeration, split transactions for FS/LS children, multi-configuration Apple search: ipheth hides in config 4). Hardware-confirmed: all 4 W540 internal devices, two-tier dock cascade, and **iPhone 0x05AC:12A8 → Trust prompt → GET_MACADDR → carrier 0x04 (hotspot active)**. Data path: persistent bulk-IN RX QH + `iphone_net.rs` NetDevice `ipheth0` + `net::init_with_ipconfig` (172.20.10.9/28, phone = gateway+DNS; DHCP bypassed) — W540 traffic validation is the next session's first move. Also landed: incremental `usbenum` (connect-change driven), VERBOSE_ENUM boot-log trim, dock-cascade re-reset+retry (Opus-agent assisted; review caught the connected-but-unrecorded gap, fixed `c152eb9`). Plan: `docs/IPHONE_SENSOR_OFFLOAD_PLAN.md` (LiDAR-over-tether + W540 device-driver triage). Commits `d11a63e`..`eef5a1f`. |
| M27 Phase 5b iters 5-8 — semos-rustc boots + runs, DEMO 80 compile still blocked 2026-06-10 | **The 88 MB statically-linked `semos-rustc` binary boots Ring-3 on real hardware (T540p), runs `_start` → `main` → `rustc_driver_impl::run_compiler`, and `--version`/`--help` work end-to-end; the actual `/hello.rs` → ELF compile silently exits with zero diagnostics — still being root-caused.** Iters 5-6: binary baked into the kernel via `include_bytes!` (100 MB image — note: never reaches serial under QEMU-BIOS, loads fine on hardware + UEFI, see [[project_semos_qemu_bios_size_limit]]), `/bin/semos-rustc` spawn arm + ramfs entry + `/hello.rs` registered in the path namespace (`Namespace::create_file`, since rustc's `RealFileLoader` reads through SYS_OPEN not the flat ramfs) + `/tmp` dir; `rustc_session::getopts` stub upgraded from no-op to a real argv parser (`--long`, `--long=val`, `-short`, `-short val`, `-Xvalue` against registered opt tuples); `semos-rustc::main` short-circuits `--version`/`--help` in user-space ahead of the driver (the diagnostic-init stack is still cfg-gate territory). **Iter 7 (the headline kernel fix): root-caused + fixed a PML4-recycle double-destroy that made the 2nd-and-later spawn of any large ELF fail with `map_user_region FAILED` / OOM panic** — `reclaim_dead_address_spaces` freed a dead AS's PML4 frame back to PT_POOL, the next spawn's `create_address_space` got the SAME physical frame back (new AS cr3 == dying cr3), then the scheduler's `reap_slot` path destroyed the new spawn's AS mid-flight via the stale `CONTEXTS[i].cr3`. Fix: scrub `CONTEXTS[i].cr3` for every slot matching the freed AS during reclaim. Also landed in iter 7: recursive `AddressSpace::destroy` page-table walker (frees all leaf + interior frames, was leaking ~22.5K frames/spawn), provenance-routed leaf free (security-pool vs PT_POOL by phys range), `MAX_PT_FRAMES` 32K→131K, `MAX_SUBTABLES` 32→256, and wiring the previously-zero-caller `reclaim_address_spaces()` into the spawn path. Validated 4+ back-to-back `semos-rustc --help` invocations on hardware. Also: sem-sh up/down-arrow history now repaints the line (kernel cooked-mode line discipline already navigated the buffer; the redraw was missing); `SYS_TTY_SUPPRESS=117` + `Platform::tty_suppress` added (per-prompt defensive clear in repl). **Iter 8 (open): the real `/hello.rs -o /tmp/hello.elf` compile prints `argv:` then returns to the shell with NO diagnostics, NO pipeline checkpoints, NO panic.** Made the `Buffy` diagnostic emitter write-through (was buffering into a Vec that `process::exit` discarded before flush — a real latent bug, fixed). Added `Callbacks` checkpoint hooks + `aot_semos::run_aot` entry probe + raw `semos_std::println!` probes bracketing `rustc_driver_impl::run_compiler` (entry → handle_options → make_input) and `interface::run_compiler` (entry → jobserver → setup_callbacks → build_target_config → thread-pool closure → session build → user closure → parse). Probe strings verified present in the booted image (grep through binary→kernel→.img), but the last hardware boot still showed zero probes — meaning the exit is in the un-instrumented gap OR a session-build path that exits before the first probe; next session resumes from the probe output. **Commits: `814020e` (iters 5-7 + PML4 fix), `eb86312` (diagnostic cleanup); iter-8 probes uncommitted.** New gotchas captured: [[bug_pml4_recycle_double_destroy]], [[project_semos_qemu_bios_size_limit]], [[feedback_two_step_kernel_image_build]], [[feedback_rustup_toolchain_env]], [[feedback_semos_rustc_fingerprint_stale]] (vendored-rustc edits sometimes don't trigger rebuild on Windows — verify binary mtime, force by deleting the crate's rlib+fingerprint). |
| M27 Stage H iters 2-4 — rustc_driver tier CLOSED 2026-06-08 | **8 of 8 originally-broken `rustc_*` crates target-buildable on x86_64-unknown-none (commits `777b89a` → `7b75379`).** Iter 2 (`777b89a`): rustc_interface 416 → 16 via cfg-split fs/io/path/env/sync imports + add semos-std as target-only dep + cfg-gate `run_in_thread_with_globals` / `get_codegen_backend` / dylib loader / `DummyArchiveBuilderBuilder` / `link_binary` to host-only (target paths get stubs); env_var_os cfg-split with str::from_utf8_unchecked through tcx arena. Iter 3 (`8af701f`): rustc_interface 16 → 0 — `Box::leak` gcx_cell/arena/hir_arena on target to dodge a `for<'tcx>` borrow-checker conservatism; cfg-gate `serialize_rlink` (host-only rlink path) + IsTerminal + diagnostic file checksums. Iter 4 (`7b75379`): rustc_driver_impl 342 → 0 + rustc_driver 2 → 0 — same alloc-prelude sweep + strip ~190 `#[instrument]` + cfg-gate panic_backtrace_config / panic_update_hook features + cfg-split top-level fs/io/path/process/sync/ffi/env/time imports + cfg-gate ~280 LOC of panic-hook + ICE + signal handling + show_md_content_with_pager + mod highlighter + json_schema + DEFAULT_LOCALE_RESOURCE + io::stdin + rustc_errors::markdown re-export + PathBuf::from(&String) → PathBuf::from(o.as_str()); extend rustc_session::getopts target stub with Options::{parse, usage, usage_with_format} + Fail enum; rustc_driver wrapper gets `#![cfg_attr(target_os = "none", no_std)]`. Cumulative Stage H: 1436 + ~2200 = ~3600 errors → 0. The full rustc compiler infrastructure now compiles target-side; Phase 5b can wire `rustc_driver_impl::run_compiler` into `semos-rustc::main` for DEMO 80. |
...
## M27 — First rustc build on Semantic OS `[🔨 Phase 5c Stage G + H both CLOSED — full rustc_* tier target-buildable on x86_64-unknown-none. Stage G iters 6–10 closed the Cranelift stack + cg_clif + emitted a 6.1 MB ELF from the semos-rustc binary. Stage H iters 1–4 closed 8 previously-broken rustc_* crates including rustc_driver_impl + rustc_driver via alloc-prelude sweep + cfg-gates for host-only subsystems (panic hooks, dylib loader, thread pool, ICE handler, pager, anstyle, getopts). Next: Phase 5b wires rustc_driver_impl into the semos-rustc binary for DEMO 80 (compile-and-spawn hello-world on SemOS end-to-end)]`
