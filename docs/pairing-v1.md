...
Pin fixed inputs so Rust and Swift stay bit-identical (the M55 acceptance gate,
mirroring the crypto-KAT discipline):

```
phone_static_priv = 01 00 … 00   (32B, little-endian scalar per RFC 7748)
sem_static_priv   = 02 00 … 00
nonce_p           = "PPPPPPPPPPPPPPPP" (0x50 ×16)
nonce_s           = "SSSSSSSSSSSSSSSS" (0x53 ×16)
version           = 0x01
⇒ phone_pub, sem_pub, shared, prk, session_key, SAS(6 digits), pairing_id
```

Fill the outputs by running the SemOS Rust implementation first (it is the
reference), commit them here, then make the Swift `CryptoKit` implementation
reproduce them exactly before any on-device pairing is attempted.
