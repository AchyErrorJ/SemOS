...
**4.4 Vendored forks with no update policy.** `kernel-core/vendor/embedded-tls` (a fork, with local shim `tls/crypto_shim.rs`), `compiler/vendor/` (Cranelift 0.122 et al., ~550k LoC), `user-programs/semos-rustc` (~1.13M LoC). Vendoring is the right call for `no_std` patching — but there's no `VENDORING.md`, no recorded upstream revision/diff, no update cadence. For a TLS stack especially, "which upstream commit + which local patches" needs to be written down, or security fixes will never find their way in.
...
6. **P2 — `VENDORING.md`** recording upstream revs + local patches, starting with embedded-tls; add the Intel firmware license file.
