...
#### D.2-followup — live cranelift-codegen on SemOS **DONE** (DEMO 73 live, 2026-05-30)
`cranelift-codegen` 0.122 + `cranelift-frontend` 0.122 now build clean
against `x86_64-unknown-none` + `semos-std` and run Ring-3 on SemOS.
`semos-cc/src/main.rs` no longer carries `const ADD_BYTES` — it
constructs the IR via `FunctionBuilder`, verifies, lowers via
`Context::compile`, and splices the live-generated bytes into the
emitted ET_EXEC. DEMO 73 stage A: semos-cc exits 0 with all 5 codegen
stages logged. Stage B: the emitted ELF runs the live-Cranelift-produced
`add(1, 2)` and exits 3.

The port itself: 7000+ errors → 0 across ~14 vendored crates. Full
per-crate detail (and the lessons learned — host vs target build
separation, workspace traps, hashbrown's `default-hasher`, libm shim
for f32/f64 methods, OnceLock shim) lives in `user-programs/semos-cc/
PORT_LOG.md`. Two kernel-side tuning changes were also load-bearing:
`MAX_PT_FRAMES` 2048→32768 (128 MiB pool, the 5.4 MiB Cranelift-bearing
semos-cc needs the headroom) and `USER_PROC_STACK_SIZE` 64 KiB→1 MiB
(Cranelift's regalloc recursion blew the small stack; the resulting
overflow scribbled past the exit frame and showed up as a mysterious
`0xFA00_0497` exit code before the bump made it go away).

Closes the M27 D.2 open follow-up. Next D-step is D.3 (a parser
front-end so the compiler can take novel source instead of a hardcoded
single IR), per Session D's "skip rustc entirely and use a simpler
front-end" branch.
