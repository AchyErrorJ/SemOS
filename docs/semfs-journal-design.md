...
FAT-write, not a page cache. It is also **not Sheaf** — Sheaf
(`docs/SHEAF_PLAN.md`) is the bundle *content model* (userland prototype);
SemFS is the kernel object store this journal makes durable. Sheaf bundles
would eventually live as SemFS objects, but the two are separate layers.
