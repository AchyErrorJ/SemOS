// M27: gated off by `#[cfg(all(test, feature = "rustc_arena_tests"))]` in lib.rs.
// Pulls `extern crate test` + `thread_local!` + std-only constructs (String/Vec
// usable here since alloc is in scope, but the test harness itself needs std).
// Kept verbatim from upstream so we can re-run host-side benches against this
// crate via `cargo test --features rustc_arena_tests` on a std toolchain.
//
// On `x86_64-unknown-none` (SemOS target) this file is excluded by the cfg
// above. Do not edit this file's body without verifying both host-test and
// SemOS-target builds.
