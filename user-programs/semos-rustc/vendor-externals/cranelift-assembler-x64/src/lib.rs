...
// M27 Phase 5c Stage G iter 5b: no_std on SemOS target (x86_64-unknown-none).
// Upstream cranelift-assembler-x64 has no `#![no_std]` declaration and
// no std/core/alloc features — assumes std. The unconditional `use std::*`
// lines in mem.rs/api.rs/imm.rs/etc. break against `x86_64-unknown-none`
// which has NO `std` crate. Switch to no_std + alloc + add `extern crate
// std;` host-only so dev-deps still work on the build host.
