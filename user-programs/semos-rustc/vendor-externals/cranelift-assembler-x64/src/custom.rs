...
pub mod mnemonic {
    ...
    // M27 Phase 5c Stage G iter 5b: no_std prelude.
...
pub mod display {
    ...
    // M27 Phase 5c Stage G iter 5b: no_std prelude. `ToString` is needed
    // because the generator emits `.to_string()` on Imm8/Imm32/Amode/etc.
    // that takes the blanket impl via the `Display` impls.
