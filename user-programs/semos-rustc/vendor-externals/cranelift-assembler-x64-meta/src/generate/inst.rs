...
impl dsl::Inst {
    ...
    pub fn generate_mnemonic_function(&self, f: &mut Formatter) {
        ...
        fmtln!(f, "#[must_use]");
        fmtln!(f, "#[inline]");
        // M27 Phase 5c Stage G iter 5b: emit alloc::borrow::Cow rather than
        // std::borrow::Cow so the consumer crate (cranelift-assembler-x64)
        // builds on x86_64-unknown-none.
        f.add_block(
    ...
    pub fn generate_display_impl(&self, f: &mut Formatter) {
        ...
        let impl_block = self.generate_impl_block_start();
        let struct_name = self.struct_name_with_generic();
        // M27 Phase 5c Stage G iter 5b: emit core::fmt rather than std::fmt.
        f.add_block(
