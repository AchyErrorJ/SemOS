...
// M27 Phase 5c Stage G iter 4c: drop `feature = "write"` from the std cfg.
// write now uses core/alloc + rustc_hash::FxBuildHasher instead of
// std-only RandomState. x86_64-unknown-none has no std crate at all, so
// even `extern crate std` triggers E0463 here.
#[cfg(feature = "std")]
#[macro_use]
extern crate std;
