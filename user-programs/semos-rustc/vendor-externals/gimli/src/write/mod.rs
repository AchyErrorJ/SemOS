...
// M27 Phase 5c Stage G iter 4c: pin IndexSet/IndexMap to FxBuildHasher so
// the write modules compile under no_std. indexmap's `S = RandomState`
// default only exists with std. FxBuildHasher is `core::hash::BuildHasher`
// + Default, no allocations, deterministic.
