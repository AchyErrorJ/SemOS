...
// D.2 port: no_std FxHashMap/FxHashSet backed by hashbrown, so
// downstream crates that use these names (cranelift-codegen does so
// pervasively) work without std.
#[cfg(not(feature = "std"))]
