#!/usr/bin/env bash
# DEMO 87 harness: same pipe-serial pattern as run-demo83.sh.
# Usage: run-demo87.sh [y|n|timeout]
ANS="${1:-y}"
IMG="$HOME/SemOS/kernel-x86_64/target/x86_64-unknown-none/release/semantic-os-x86_64.img"
LOG="$HOME/demo87-serial-$ANS.log"
...
# Two prompts to answer this boot: /apps/calc (M2) then /apps/wc (M3).
seen=0
for i in $(seq 1 900); do
  ...
  if grep -qE "\[DEMO 87\] (PASS|FAIL)" "$LOG" 2>/dev/null; then break; fi
  if grep -q "\[DEMO 87\] FAIL" "$LOG" 2>/dev/null; then break; fi
  sleep 2
done
echo "answered $seen prompt(s) after ~$((i*2))s"

for i in $(seq 1 300); do
  grep -qE "\[DEMO 87\] (PASS|FAIL)" "$LOG" 2>/dev/null && break
  sleep 2
done
