...
pub struct CertificateRef<'a> {
    ...
    // VENDORED PATCH (Semantic OS): visibility relaxed from `pub(crate)` to `pub`
    // so external `TlsVerifier` impls can walk the chain. Upstream issue: an
    // implementor of the `TlsVerifier` trait outside this crate has no way to
    // get at the parsed entries — the trait passes a `CertificateRef` whose
    // only field is sealed. We need the entries to do SPKI pinning. One
    // visibility token is the smallest possible patch; we don't change any
    // logic, layout, or trait signatures, so upstream re-vendor stays trivial.
    // See kernel-core/src/tls/verifier.rs for the consumer.
