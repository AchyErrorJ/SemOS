...
/// THE killer demo: a Ring 3 ELF that contains a string with PII
/// (email, credit card, SSN), prints it raw via SYS_WRITE, calls
/// SYS_LLM_REDACT to have the kernel mediate the data, then prints the
/// redacted version. The output buffer for the redacted text lives on
/// the user stack (the only writable page in this binary).
///
/// Input:
///   "Demo PII: email=user@example.com card=4111-1111-1111-1111 ssn=555-12-3456\n"
/// Expected output (kernel's Redactor masks emails, cards, SSNs):
///   "Demo PII: email=[EMAIL] card=[CARD] ssn=[SSN]\n"
///
/// The point: the Redactor lives in ring 0. Even a tier-0 (Public) user
/// task that owns its own data cannot bypass the redaction layer when it
/// asks the kernel to send data to "the LLM" — the kernel sanitizes it
/// at the syscall boundary. Same primitive that would gate any future
/// SYS_LLM_QUERY, SYS_LLM_CONTEXT, or SemanticObject access.
///
/// File layout (single 512-byte PT_LOAD at vaddr 0x400000, R+X):
///   0x00-0x3F: ELF header
///   0x40-0x77: program header + padding
///   0x78:      code (entry) — ~86 bytes
///   0x180:     raw PII string ("Demo PII: ...", 74 bytes)
#[allow(dead_code)]
pub fn create_redact_elf() -> [u8; 512] {
    let mut buf = [0u8; 512];
    ...
    buf[ph + 16..ph + 24].copy_from_slice(&0x400000u64.to_le_bytes()); // p_vaddr
    buf[ph + 24..ph + 32].copy_from_slice(&0x400000u64.to_le_bytes()); // p_paddr
    buf[ph + 32..ph + 40].copy_from_slice(&512u64.to_le_bytes());      // p_filesz
    buf[ph + 40..ph + 48].copy_from_slice(&512u64.to_le_bytes());      // p_memsz
    buf[ph + 48..ph + 56].copy_from_slice(&0x1000u64.to_le_bytes());   // p_align

    // ---- Embedded raw PII string at file offset 0x180 / vaddr 0x400180 ----
    let secret: &[u8] =
        b"Demo PII: email=user@example.com card=4111-1111-1111-1111 ssn=555-12-3456\n";
    const STR_OFFSET: usize = 0x180;
    const STR_VADDR: u64 = 0x400000 + STR_OFFSET as u64;
    let secret_len = secret.len() as u32;
    buf[STR_OFFSET..STR_OFFSET + secret.len()].copy_from_slice(secret);

    // ---- Code at file offset 0x78 / vaddr 0x400078 ----
    // Sequence (~86 bytes):
    //   ; --- Print raw secret ---
    //   48 BF <8B>      mov rdi, STR_VADDR        ; arg0 = buf_ptr
    //   BE <4B>         mov esi, secret_len       ; arg1 = buf_len
    //   B8 00 00 00 00  mov eax, 0                ; SYS_WRITE
    //   0F 05           syscall
    //   ; --- Redact via kernel ---
    //   48 81 EC <4B>   sub rsp, 256              ; alloc output buf on stack
    //   48 BF <8B>      mov rdi, STR_VADDR        ; arg0 = input ptr
    //   BE <4B>         mov esi, secret_len       ; arg1 = input len
    //   48 89 E2        mov rdx, rsp              ; arg2 = output ptr (stack)
    //   B8 34 00 00 00  mov eax, 52               ; SYS_LLM_REDACT
    //   0F 05           syscall                   ; rax = redacted length
    //   ; --- Print redacted ---
    //   49 89 C2        mov r10, rax              ; preserve length
    //   48 89 E7        mov rdi, rsp              ; arg0 = output ptr
    //   4C 89 D6        mov rsi, r10              ; arg1 = length
    //   B8 00 00 00 00  mov eax, 0                ; SYS_WRITE
    //   0F 05           syscall
    //   ; --- Restore stack & exit ---
    //   48 81 C4 00 01 00 00   add rsp, 256
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        // mov esi, secret_len
        buf[p] = 0xBE; p += 1;
        buf[p..p + 4].copy_from_slice(&secret_len.to_le_bytes()); p += 4;
        // mov eax, 0  (SYS_WRITE)
        buf[p] = 0xB8; p += 1;
        buf[p..p + 4].copy_from_slice(&0u32.to_le_bytes()); p += 4;
        // syscall
        ...
        // sub rsp, 256
        buf[p] = 0x48; buf[p + 1] = 0x81; buf[p + 2] = 0xEC; p += 3;
        buf[p..p + 4].copy_from_slice(&256u32.to_le_bytes()); p += 4;
        // mov rdi, STR_VADDR (again — was clobbered if anything moved it)
        buf[p] = 0x48; buf[p + 1] = 0xBF; p += 2;
        buf[p..p + 8].copy_from_slice(&STR_VADDR.to_le_bytes()); p += 8;
        // mov esi, secret_len
        buf[p] = 0xBE; p += 1;
        buf[p..p + 4].copy_from_slice(&secret_len.to_le_bytes()); p += 4;
        // mov rdx, rsp
        buf[p] = 0x48; buf[p + 1] = 0x89; buf[p + 2] = 0xE2; p += 3;
        // mov eax, 52  (SYS_LLM_REDACT)
        buf[p] = 0xB8; p += 1;
        buf[p..p + 4].copy_from_slice(&52u32.to_le_bytes()); p += 4;
        // syscall  (rax = redacted length)
        buf[p] = 0x0F; buf[p + 1] = 0x05; p += 2;

        // mov r10, rax
        buf[p] = 0x49; buf[p + 1] = 0x89; buf[p + 2] = 0xC2; p += 3;
        // mov rdi, rsp
        buf[p] = 0x48; buf[p + 1] = 0x89; buf[p + 2] = 0xE7; p += 3;
        // mov rsi, r10
        buf[p] = 0x4C; buf[p + 1] = 0x89; buf[p + 2] = 0xD6; p += 3;
        // mov eax, 0  (SYS_WRITE)
        buf[p] = 0xB8; p += 1;
        buf[p..p + 4].copy_from_slice(&0u32.to_le_bytes()); p += 4;
        // syscall
        ...
        // add rsp, 256
        buf[p] = 0x48; buf[p + 1] = 0x81; buf[p + 2] = 0xC4; p += 3;
        buf[p..p + 4].copy_from_slice(&256u32.to_le_bytes()); p += 4;
        // xor edi, edi
        buf[p] = 0x31; buf[p + 1] = 0xFF; p += 2;
        // mov eax, 2  (SYS_EXIT)
